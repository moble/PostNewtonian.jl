...
"""
@compute_pn_variables 2 function h!(h, pnsystem::AbstractPNSystem{T}; ℓmin=0, ℓmax=typemax(Int)) where T
    ...
    if ℓmax≥3
        h̃₃₀ = c * v^4 * (√42 * (-17Sλ*δ + Σλ*(35ν - 9)) / 168M^2)
        h[Yindex(3,0,ℓmin)] += h̃₃₀
        h̃₃₁ = c * v^3 * (√14 * (𝒾*Sλ + Sₙ + δ*(𝒾*Σλ + Σₙ)) / 21M^2)
        h[Yindex(3,1,ℓmin)] += h̃₃₁
        h[Yindex(3,-1,ℓmin)] += conj(h̃₃₁)
        h̃₃₂ = c * v^4 * (√35 * (-Σλ*(83ν - 17) + 4𝒾*Σₙ*(55ν - 13) + 25δ * (Sλ - 4𝒾*Sₙ)) / 168M^2)
        h[Yindex(3,2,ℓmin)] += h̃₃₂
        h[Yindex(3,-2,ℓmin)] += conj(h̃₃₂)
        h̃₃₃ = c * v^3 * (√210𝒾 * (Sλ + 𝒾*Sₙ + δ*(Σλ + 𝒾*Σₙ)) / 21M^2)
        h[Yindex(3,3,ℓmin)] += h̃₃₃
        h[Yindex(3,-3,ℓmin)] += conj(h̃₃₃)
    end
    if ℓmax≥4
        h̃₄₀ = c * v^4 * (√2𝒾 * (Sₙ*δ - 3Σₙ*ν + Σₙ) / 168M^2)
        h[Yindex(4,0,ℓmin)] += h̃₄₀
        h̃₄₂ = c * v^4 * (√5 * (-13Σλ*(3ν - 1) + 14𝒾*Σₙ*(3ν - 1) + δ*(13Sλ - 14𝒾*Sₙ)) / 168M^2)
        h[Yindex(4,2,ℓmin)] += h̃₄₂
        h[Yindex(4,-2,ℓmin)] += -conj(h̃₄₂)
        h̃₄₄ = c * v^4 * (9√35 * (-3Σλ*ν + Σλ - 𝒾*Σₙ*(3ν - 1) + δ*(Sλ + 𝒾*Sₙ)) / 56M^2)
