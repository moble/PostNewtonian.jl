# Base.ismutable(pnsystem::PNSystem{NT, ST}) where {NT, ST} = ismutable(state(pnsystem))
# Base.ismutabletype(::Type{<:PNSystem{NT, ST}}) where {NT, ST} = ismutabletype(ST)
...
# Indexing
Base.getindex(pnsystem::PNSystem, i::Int) = @propagate_inbounds getindex(state(pnsystem), i)
Base.setindex!(pn::PNSystem, v, i::Int) = @propagate_inbounds setindex!(state(pn), v, i)
